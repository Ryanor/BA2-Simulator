...
device1.on('stateChange', function(state) {
  ...
  if (state === 'poweredOn') {
    console.log("Advertising started");
	   ...
	   device2.startAdvertising('SecondDevice',['56ab']);
...
device2.on('accept', function(clientAddress) {
    console.log("Accepted connection from address: " + clientAddress);
});
...
device1.on('disconnect', function(clientAddress) {
 ...
 device2.on('disconnect', function(clientAddress) {
    console.log("Disconnected from address: " + clientAddress);
});
...
device2.on('advertisingStart', function(error) {
    if (error) {
        console.log("Advertising start error:" + error);
    } else {
        console.log("Advertising start success");
        device2.setServices([
            ...
            // Define a new service
            new device2.PrimaryService({
                uuid : '56ab',
                characteristics : [
                    
                    // Define a new characteristic within that service
                    new device2.Characteristic({
                        value : null,
                        uuid : '78cd',
                        properties : ['notify', 'read', 'write'],
